...
## #13 匯出 OpenAPI 規格快照
- **做了什麼**：稽核時發現根 README 的專案結構寫著 `/docs` 放 OpenAPI 規格，但 `docs/` 實際只有領域模型說明，規格必須啟動後端才看得到。從執行中的後端匯出 `docs/openapi.json` 納入版控，並更新根 README 的存取說明與重新匯出指令。
- **為什麼用 AI**：比對題目繳交要求與 repo 實際內容，找出文件與實作不一致的地方。
- **檔案**：`docs/openapi.json`、`README.md`（無程式碼註解，屬文件調整）
